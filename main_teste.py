...
# EXTRACAO DOS DADOS DO ARQUIVO .CSV
...
#EXTRACAO DOS DADOS DO ARQUIVO .TXT
...
def load_data(df, path, partition_col="data_venda"):
    try:
        # 1. Criamos o writer sem o coalesce aqui
        writer = df.write.mode("overwrite").option("header", "true")
        
        if partition_col:
            # Ao particionar, o Spark já organiza os arquivos dentro das pastas
            writer.partitionBy(partition_col).csv(path)
        else:
            # Se não houver partição, aí sim usamos coalesce para ter um arquivo só
...
def main():
    ...
    try:
        # ETL
        df_cli, df_ven = extract_CSV(spark, PATH_CLIENTES), extract_TXT(spark, PATH_VENDAS)
